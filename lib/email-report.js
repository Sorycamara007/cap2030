...
export function buildReportEmailHtml({ profile = {}, report = {} }) {
  ...
  const metaBits = (r) =>
    [
      r.rattachement ? `Axe · ${esc(r.rattachement)}` : '',
      r.type ? `Type · ${esc(r.type)}` : '',
      r.duree ? `Durée · ${esc(r.duree)}` : '',
      r.modules ? `Modules · ${esc(r.modules)}` : '',
    ]
      .filter(Boolean)
      .join(' &nbsp;·&nbsp; ');
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%">
        <tr>
          ...
          <td valign="top">
            ...
            ${
              metaBits(r)
                ? `<div style="font-size:11px;letter-spacing:1px;text-transform:uppercase;color:${muted};margin-bottom:8px;">${metaBits(r)}</div>`
                : ''
            }
            <div style="font-family:Georgia,'Times New Roman',serif;color:${navy};font-size:14px;line-height:1.6;margin-bottom:8px;">${esc(r.justification || '')}</div>
            ${
              r.url
                ? `<a href="${esc(r.url)}" style="font-size:11px;letter-spacing:1px;text-transform:uppercase;color:${gold};font-weight:700;text-decoration:underline;">Voir sur catalogue.cfpc.net →</a>`
                : ''
            }
          </td>
        </tr>
...
export function buildReportEmailText({ profile = {}, report = {} }) {
  ...
  lines.push('4. RECOMMANDATIONS FORMATION (catalogue CFPC)');
  lines.push('---------------------------------------------');
  recommandationsFormation.forEach((r, i) => {
    ...
    if (r.modules) lines.push(`    Modules : ${r.modules}`);
    if (r.url) lines.push(`    Lien    : ${r.url}`);
